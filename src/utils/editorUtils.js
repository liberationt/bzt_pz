 ...
 */
 ...
 export default {
     mobileWidth: 480,           // 手机移动端页面宽度
     platformCategory:3,         // 平台的项目业务ID
     breakpageWidth: 210,        // 编辑器正文容器宽度
     breakpageHeight: 297,       // 编辑器正文容器高度
     pageData: {type:'sss'},
     ...
     getPageClassStyle(number=1) {
         return {
             ...
             style: number % 2 === 0 ? 'padding: 35mm 25mm 20mm 20mm;' : 'padding: 35mm 20mm 20mm 25mm;'
         }
     },
   // 改变DOM的标签
   ...
   getFirstNodeInContainer(container=null) {
      ...
      * @description 过滤元素集
      */
     filterNodesBySelector(nodes=[], filters=['#text','SPAN','EM','STRONG','A','BR']) {
         var data = {
             selections: [],
             filters: []
         };
      ...
      */
     getParentBySelector(node, className=undefined) {
         var returnParentElement = null;
         const getpNode = (element, className) => {
             if(element) {
                 //创建父级节点的类数组
                 let pNode = element.parentElement;
                 if(pNode && this.hasClass(pNode, className)) {
   ...
   getNextAllBySelector(container, currNode, selector) {
         const nextAll = container.querySelectorAll(selector);
         const lists = [];
         let flag = false;
   ...
   // 获取节点后的所有兄弟节点
   getNextAllNodes(node=null, selector=undefined) {
   ...
   // 获取节点前的所有兄弟节点
   getPrevAllNodes(node=null, selector=undefined) {
     ...
     findNodeBySelector(nodes=[], selector='') {
         for(let i=0; i<nodes.length; i++) {
             if(this.hasClass(nodes[i], selector)) {
                 return nodes[i];
             }
         }
         return null;
   ...
   /**
     ...
     /**
      * 重构大纲树 sam.shen add 2021-06-13
      * @param {Object} data             // 新节点数据
      * @param {Object} node             // 原始节点数据
      * @param {Object} outlineId
      */
     parseStructData(data, outlineId) {
         let newArr = [];
         let nodeId = outlineId + 1;
         const setStruct = (nodeData, isFirst=false) => {
             let children = nodeData.children.map((item, idx) => {
         if(!item.id) {
           nodeId++;
         }
                 let infoId = nodeData.infoId + "." + (idx+1);
                 if(data.appendix && isFirst) {
                     infoId = nodeData.letter + "." + (idx+1);
                 }
                 let obj = {
                     id: item.id || nodeId,
                     pid: nodeData.id,
   ...
   dateTrans(value, type) {
     switch(type) {
       case 'date':
         return this.formatDateTime('yyyy-MM-dd', new Date(value));
       case 'timestamp':
         return this.formatDateTime('yyyy-MM-dd hh:mm:ss', new Date(value));
       case 'time':
         return this.formatDateTime('hh:mm:ss', new Date(value));
     }
   },
