  ...
  <div class="enterprise-std app-container">
    ...
    <div class="enterprise-std-content">
      ...
      <div class="operation-line" v-if="taskInfo != null">
        ...
        <template
          v-if="
            ((taskInfo.specialRevise == 1 && taskInfo.taskState == 6101) ||
              taskInfo.taskState == 6101) &&
            taskInfo.workItem &&
        ...
        <template v-if="showBtn([6102, 6103, 6112]) == true">
          <div class="submit-btn" @click="auditPro(true)">审核项目</div>
        </template>

        <!-- 分发复审任务 -->
        <template v-if="showBtn([6104]) == true">
          <div class="submit-btn" @click="handlePersion(true)">
            审核分发复审任务
          </div>
          <!-- <div class="submit-btn" @click="distributePro(true)">
            审核分发复审任务
          </div> -->
        </template>

        <!-- 部门上传复审结果 -->
        <template v-if="showBtn([14001]) == true">
          <!-- 原始的 -->
        ...
        <!-- 部门分管领导审核 -->
        <template v-if="showBtn([14002]) == true">
          <!-- 原始的 -->
        ...
        <template v-if="showBtn([14005]) == true">
          <!-- 原始的 -->
          <!-- <div class="submit-btn" @click="exportFile(3)">
            导出复审结论汇总表
          </div>
        ...
        <!-- 新增 -->
        <template v-if="showBtn([14006]) == true">
          <div class="green-btn" @click="handleFile(true,'上传会议纪要')">上传会议纪要</div>
        </template>

        <template v-if="showBtn([14007]) == true">
          <div class="green-btn" @click="handleFile(true,'部门领导审核')">部门领导审核</div>
        </template>
         
        <!-- 汇总各部门复审结论 -->
        <template v-if="showBtn([6107]) == true">
          <div class="submit-btn" @click="exportFile(2)">导出复审结论汇总表</div>
          <!-- <div class="green-btn" @click="uploadResultPro(true,'上传会议纪要')">
            上传会议纪要
          </div> -->
          <div class="green-btn" @click="uploadResultPro(true, '上传结论')">
            上传结论
          </div>
          <!-- <div class="yellow-btn" @click="checkResult(true)">
            查看各部门处理情况
          </div> -->
        </template>
        ...
        <template v-if="showBtn([6108]) == true">
          <div class="submit-btn" @click="exportFile(0)">导出复审结论汇总表</div>
          <div
            class="green-btn"
            @click="uploadResultPro(true, '起草复审结果通知')"
          >
            起草复审结果通知
          </div>
        </template>
      ...
      >
        <el-table-column type="selection" :selectable="selected" width="55" v-if="showBtn([14005]) == true"> </el-table-column>
    ...
    <HandleFile
      ...
      :taskInfo="taskInfo"
      :title="title"
      :childTaskStatus="childTaskStatus"
      @save="handleFile"
    />
...
export default {
  ...
  methods: {
      ...
      Api.getRwProjectListByTaskId({
        ...
        if (code === 200) {
          ...
          if (this.list.length > 0) {
            this.childTaskStatus = this.list[0].childTaskStatus;
            ...
            this.taskInfo["childTaskId"] = this.list[0].childTaskId
              ...
              : null;
            this.taskInfo["firstAuditUser"] = Number(
              this.list[0].tRwTask.firstAuditUser
            );
            this.taskInfo["secondAuditUser"] = Number(
              this.list[0].tRwTask.secondAuditUser
            );
            this.taskInfo["startTime"] = this.list[0].tRwTask.startTime;
            this.taskInfo["endTime"] = this.list[0].tRwTask.endTime;
            this.taskInfo["lastFirstAuditUser"] = this.list[0].tRwTask
              .lastFirstAuditUser
              ? Number(this.list[0].tRwTask.lastFirstAuditUser)
              : "";
            this.taskInfo["lastSecondAuditUser"] = this.list[0].tRwTask
              .lastSecondAuditUser
              ? Number(this.list[0].tRwTask.lastSecondAuditUser)
              : "";
          }
        } else {
    ...
    eidtTask(e) {
      if (e == true) {
        ...
        if (e != null) {
          ...
          this.$router.push("/enterReview/enterReview");
        }
      }
    },
    /**
     ...
     */
    cheeckReson(e) {
      if(this.taskInfo.taskState ==14006){
        this.projectId = this.list[0].childTaskId
        this.projectType = 13
      }
    ...
    getFile(type) {
      ...
      Api.rwProcessorTaskAssignList(req).then((res) => {
        ...
        if (code == 200) {
          console.log(rows);
          if (rows.length > 0) {
            let currentAttachments = rows.filter((item)=>item.currentAttachments)
            const files = JSON.parse(currentAttachments[0].currentAttachments);
            files.forEach((file) => {
              if (file.type == 1) {
