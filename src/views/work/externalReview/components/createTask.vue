    ...
    <template slot="body">
      ...
      <div style="display:flex;flex-wrap:wrap">
        ...
        <el-form-item
            ...
            prop="firstAuditUser"
          >
          <el-select v-model="taskForm.firstAuditUser" filterable clearable placeholder="请选择标准化业务主管" :disabled="disabled" style="width: 360px;">
...
export default {
  name: "createTask",
  components: { BztDialog, BztTable, CheckProjectList,TecCommittees,BztUploadFile },
  props: {
    visiable: {
      type: Boolean,
      default: false,
  ...
  watch: {
    visiable(val) {
      ...
      if (this.visiable === true) {
        console.log(this.taskDetail,111)
        this.initPage();
      }
    },
  },
  mounted() {
    // console.log(this.taskDetail,999)
    this.dialogInfo.visible = this.visiable;
    if (this.visiable === true) {
      this.initPage();
    }
  ...
  methods: {
    fileChange(file, name) {
      if(file&&file.length){
        this.taskForm.fileList = JSON.stringify([{
          url:file[0].url,
          name:file[0].name
        }])
    ...
    initPage() {
      ...
      if (this.taskInfo != null) {
        ...
        this.taskForm.firstAuditUser = Number(this.taskInfo.firstAuditUser);
        this.taskForm.secondAuditUser = Number(this.taskInfo.secondAuditUser);
        this.taskForm.ownerCompanyId = this.taskInfo.ownerCompanyId;
        this.taskForm.dataTime = [startTime.slice(0,10),endTime.slice(0,10)]
        this.filePath = [
          {
            name:this.taskDetail[0].scpAttachmentFile[0].name,
            url:this.taskDetail[0].scpAttachmentFile[0].url
          }
        ]
        this.taskForm.fileList = JSON.stringify( this.filePath)
