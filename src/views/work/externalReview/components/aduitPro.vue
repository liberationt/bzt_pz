  ...
  >
    <template slot="body">
      ...
      >
        ...
        <el-form-item label="通知内容" v-if="taskInfo.taskState == 7109">
          {{ taskInfo.note }}
        </el-form-item>
        <el-form-item label="附件" v-if="taskInfo.taskState == 7109">
          <span class="link-text" @click="getFile()">
            《企业标准复审结论汇总表》.xlsx
          </span>
        </el-form-item>
        <el-form-item label="审核结论" prop="state" v-if="taskInfo.childTaskState!=15002">
      ...
      <Table v-if="taskInfo.childTaskState==15002" :taskState="taskInfo.childTaskState"  :tableData="tableData" :disabled="true" />
    </template>
  </BztDialog>
...
export default {
  ...
  mounted() {
     if(this.taskInfo.childTaskState==15002){
      this.getInfo()
      ...
      if (e === true) {
        this.$refs.auditProForm.validate((valid) => {
          if (valid) {
            ...
            if (
              this.taskInfo.taskState == 7102 ||
              this.taskInfo.taskState == 7103  
            ) {
              this.auditProForm.type =
                this.taskInfo.taskState == 7102
                  ...
                  ? 2
                  : 3;
              if (this.taskInfo.taskState == 7103) {
                this.auditProForm.specialRevise =
                  this.auditProForm.state == 2 ? 1 : 0;
              }
              this.auditProForm["projectType"] = 7;
            ...
            } else if (this.taskInfo.childTaskState == 15002) {
              // const req = {
            ...
            }else if(this.taskInfo.taskState==7100){
              const req ={
                   state: this.auditProForm.state,
                   id: this.taskInfo.id,
                   opinion: this.auditProForm.opinion,
