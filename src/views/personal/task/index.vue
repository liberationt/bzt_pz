  ...
  <div class="app-container personal-task-container">
    <el-row :gutter="10">
      ...
      <el-col :span="21">
            ...
            >
              ...
              <div class="header-btn-group">
                <span
                  ...
                  :key="item.value"
                  class="btn-link"
                  :class="{ 'btn-active': queryParams.state == item.value }"
        ...
        <!-- 任务列表 -->
        <div class="main-list">
          <el-table v-loading="loading" :data="taskList" stripe @selection-change="handleSelectionChange">
            <el-table-column
              v-if="queryParams.taskType==23"
              type="selection"
              width="50"
            ...
            >
              <template slot-scope="scope">
                ...
                {{scope.row.projectStatus?handleName(scope):scope.row.taskSubType}}
              </template>
            </el-table-column>
            <el-table-column
              label="发送人"
            ...
            >
              <template slot-scope="scope">
                ...
                <el-button
                  ...
                  v-if="
                    scope.row.state != 1 &&
                    scope.row.deptId == null &&
                    scope.row.roleId == null &&
                    scope.row.hook.length == 20 &&
                    scope.row.taskType != 4 &&
                    scope.row.taskType != 5
                  "
    ...
    <!-- <el-dialog
      class="task-dialog"
      :visible.sync="currItem.visible"
      :title="currItem.data.taskTitle"
      append-to-body
      :close-on-click-modal="false"
    >
      <component
        :is="currItem.comp"
        :todoTask="currItem.data"
        @close="handleClose"
    ...
    <BztDialog
      v-if="currItem.visible == true"
      :dialogInfo="{
        visible: currItem.visible,
        title: currItem.data.taskTitle,
        width: '900px',
      }"
      @close="currItem.visible = false"
    >
      <template slot="body">
        <component
          :is="currItem.comp"
    ...
    <!-- 批量审核 -->
    <BztDialog
      v-if="auditCurrItem.visible == true"
      :dialogInfo="{
        visible: auditCurrItem.visible,
        title: auditCurrItem.taskTitle,
        width: '900px',
      }"
      @close="currItem.visible = false"
    >
    ...
    <!-- 任务转办 -->
    <TransferToOther
      v-if="transferVisiable == true"
      :visiable="transferVisiable"
      :dataSource="currItem.data"
      @close="transferClose"
    />
  </div>
</template>
...
import  BatchAudits  from "@components/bussinessCom/model/batchAudit.vue";
import { TransferToOther } from "@components/bussinessCom/index";
export default {
  components: { BztDialog, TransferToOther,BatchAudits },
  name: "todoTask",
  data() {
    return {
      // 遮罩层
  ...
  methods: {
    ...
    /** 批量审核 */
    batchAudit(){
      if(!this.auditCurrItem.data.length){
        return this.$message({
          message: '请选择要审核的数据',
          type: 'warning'
        });
      }
      this.auditCurrItem.visible = true
    },
    /** 批量审核全选 */
    handleSelectionChange(val){
      console.log(val,99)
      this.auditCurrItem.data = val
    },
    /** 搜索按钮操作 */
    handleQuery() {
    ...
    getList() {
      this.loading = true;
      TaskApi.listTodoTask(this.queryParams).then((response) => {
        this.taskList = response.rows;
        this.total = response.total;
        this.loading = false;
        this.searchKeyword = this.queryParams.searchValue;
    ...
    handleExport() {
      this.downloadByPost(
        "system/todoTask/export",
        {
          ...this.queryParams,
        },
        `待办任务_${new Date().getTime()}.xlsx`
      );
    },
    // 获取任务统计数量
    getStatisticsCount() {
    ...
    getTypeCount() {
      TaskApi.getTypeCount().then((res) => {
        let typeOptions = this.taskTypeOptions;
        typeOptions.forEach((item, index) => {
          let countData = _.find(res.data, {
            taskType: Number(item.dictValue),
          });
    ...
    // 处理状态数量统计
    getStateCount() {
      TaskApi.getStateCountByType(this.queryParams.taskType).then((res) => {
        this.stateOptions.forEach((item) => {
          let countData = _.find(res.data, { state: item.value });
          item.count = countData ? countData.count : 0;
        });
        let allCount = 0;
    ...
    // 切换任务状态
    changeState(item) {
      this.queryParams.state = item.value;
      this.handleQuery();
    },
    // 用户名称转换
    parseUser(userId) {
    ...
    handleSuccess(msg,isclear) {
      this.$message.success(msg&&msg!=true&&msg!=false ? msg : "任务处理成功");
      if(isclear){
        this.auditCurrItem.visible=false
  ...
  created() {
    ...
    this.getDicts("todo_task_type").then((response) => {
