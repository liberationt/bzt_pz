<template>
  <div class="select-user-by-dept">
    ...
    <div class="content" v-else-if="checkDepAllUser == false">
      ...
      <!-- 新增 -->
      <div v-if="checkOneUser == false">
        <ul class="company-list" style="text-align: left; width: 240px">
          <div style="text-align:center;line-height:40px;background:#f5f7fa">
              <!-- <el-checkbox
              :indeterminate="isIndeterminate"
              v-model="checkAll"
              @change="handleCheckAllChange"
          ...
          <div class="company-user">
            <el-checkbox-group v-model="users" class="" @change="handelUesrInfo">
              <template v-for="user in userList">
                <el-checkbox  :label="user.userId" :key="user.userId" :disabled="user.disabled">{{ user.nickName }}({{ user.userName }})</el-checkbox>
              </template>
            </el-checkbox-group>
          </div>
        </ul>
        <ul class="company-list" style="text-align: left; width: 240px">
          ...
          <div class="company-user company-users">
            <template v-for="(user,i) in users">
              <el-checkbox  :label="user" :key="user" :disabled="user==disableUsers" @change="()=>handelRemoveInfo(user,i)">{{getUserName(user+'')}}</el-checkbox>
            </template>
          </div>
        </ul>
      </div>
...
export default {
  ...
  mounted() {
   if(this.hasDeptId === false){
    ...
    // this.getUserListByDept(this.deptIds)
    //新增回显问题
    if(this.isEcho&&this.disableDepts&&this.disableDepts.length){
      this.getUserByDeptsAndRoleKeys(this.disableDepts.join(","))
    }
  },
  methods: {
    //添加选择用户
    handelUesrInfo(val){
       const Index = this.userList.findIndex((item) => {return item.userId === val[val.length-1]})
       this.userList.splice(Index,1)
       this.userChange(this.users);
    },
    //去除已选择用户
    handelRemoveInfo(val,i){
      this.users.splice(i,1)
      let arr = []
      for(let i=0;i<this.cloneUserList.length;i++){
        if(this.users.indexOf(this.cloneUserList[i].userId)==-1){
          arr.push(this.cloneUserList[i])
        }
      }
      this.userList = arr
      this.userChange(this.users);
    ...
    getUserListByDept(id) {
      if (this.roleKeys == null || this.roleKeys == "") {
        ...
        listUser(req).then((res) => {
          ...
          if (code === 200) {
            this.userList = rows;
            // this.cloneUserList = this.deepClone(rows)
            ...
            //新增对比
            this.cloneUserList = this.deepClone(this.userList)
        ...
        listUserByRoleKey(req).then((res) => {
          ...
          if (code === 200) {
            this.userList = data;
            this.cloneUserList = this.deepClone(data)
          } else {
            // this.$message.error(msg);
          }
        });
    ...
    getUserByDeptsAndRoleKeys(id,checkDepts=null) {
      ...
      listUserByDeptsAndRoleKeys(req).then((res) => {
        ...
        if (code === 200) {
          ...
          // }
          //去除网员逻辑
          if(this.isDelNetwork){
            if (this.checkDepAllUser == false) {
              let dataList = []
              data.forEach((item)=>{
                let flag = false
                if(item.roles&&item.roles.length){
                  flag = item.roles.some(item=>item.roleKey=='stdLiaison')
                }
                if(flag==false){
                  dataList.push(item)
                }
          ...
          if (this.hasDeptId == false) {
            ...
            // this.handelUesrInfo(this.users)
          } else {
            this.checkUserList.forEach((user) => {
              userArr.push({ depId: user.deptId, userId: user.userId });
            });
          ...
          if(checkDepts){
            for(let i=0;i<this.users.length;i++){
              let index = this.userList.findIndex((item)=>{return item.userId==this.users[i]})
              this.userList.splice(index,1)
            }
...
.select-user-by-dept {
  ...
  .company-list {
    ...
    .company-user{
      ...
      ::v-deep .el-checkbox-group .el-checkbox__label, ::v-deep .el-checkbox .el-checkbox__label{
        padding-left: 5px;
        width:160px;
        white-space: nowrap;
        /* overflow 溢出部分隐藏 */
