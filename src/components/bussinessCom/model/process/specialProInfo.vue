<template>
  <div class="special-pro-info">
      ...
      >
        ...
        >
          <el-select
            ...
            style="width: 100%"
            v-if="isEdit == true && formValue.id == null"
          >
        ...
        <el-form-item
          ...
          style="width: 100%"
          :rules="[
            { required: true, message: '特殊流程申请原因', trigger: 'blur' },
          ]"
        >
          <el-input
            type="textarea"
            autosize
            placeholder="请输入特殊流程申请原因"
            v-model="formValue.specialApplyReason"
            v-if="isEdit == true"
          />
          <span v-else>{{ formValue.specialApplyReason }}</span>
        </el-form-item>
        <el-form-item
          label="项目名称"
          style="width: 100%"
        ...
        <el-form-item label="接口对接人" style="width: 100%">
          <el-input
            type="textarea"
            autosize
            placeholder="请输入接口对接人"
            :value="getUserName(formValue.interfacePersonIds)"
            readonly
            @focus="changeInterfacePerson()"
            v-if="isEdit == true"
          />
          <span v-else>{{ getUserName(formValue.interfacePersonIds) }}</span>
        </el-form-item>
    ...
    <!-- 选择标准化对接人 -->
    <BztDialog :dialogInfo="dialogInterFacePerson" @close="saveInterFacePerson">
      <template slot="body">
        <SelectUserByDept
          :value="formValue.interfacePersonIds"
...
export default {
  ...
  methods: {
     ...
     * 打开接口对接人选择窗口
     */
    changeInterfacePerson() {
      this.users = "";
      if (this.isEdit == true) {
        this.users = this.formValue.interfacePersonIds;
    ...
    saveInterFacePerson(e) {
      if (e == true) {
        this.formValue.interfacePersonIds = this.users;
      }
